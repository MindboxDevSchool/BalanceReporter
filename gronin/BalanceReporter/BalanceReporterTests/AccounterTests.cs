...
namespace BalanceReporterTests
{
    public class AccounterTests
    {
        ...
        [Test]
        public void TransactionsPerMonth_return_container_with_correct_elements()
        {
            var transactions = account.TransactionsPerMonth(2000, 2);
            var transArray = transactions as Transaction[] ?? transactions.ToArray();
            string[] senders = {"TWO", "TWO", "THREE"};

            for (int i = 0; i < senders.Length; i++)
            {
                Assert.AreEqual(senders[i], transArray[i].Sender);
            }
        }
